...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    for (let it = 0; it < 100; ++it){
      ...
      let transportId = Math.floor(Math.random() * (length_of_types_of_transport))
      
      // make checks and make depart != dest
      if (transportId == length_of_types_of_transport)
        transportId -= 1
  ...
  down: async (queryInterface, Sequelize) => {
     ...
     */
     db.sequelize.query('SET FOREIGN_KEY_CHECKS = 0', null, {raw: true}).then(async function(results){
